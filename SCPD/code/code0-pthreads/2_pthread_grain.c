...
grain -> numeri di elementi associati ai vari thread, grain = n parte un solo thread
ogni thread fa un for da 0 a grain -1 e ne calcola la somma parziale
dopodichè i thread sommano le varie somme in total sum -> più scrittori più lettori quindi usiamo una lock
*/
...
double diffmsec(struct timeval *a, struct timeval *b) {
    long sec  = (a->tv_sec  - b->tv_sec); // differenza in secondi
    long usec = (a->tv_usec - b->tv_usec); // differenza in microsecondi
...
typedef struct arg_struct { // parametri da passare ai vari thread, allocata nell'heap, liberata nel thread
    long *a; 	// puntatore al sub array -> solo lettura nessuna scrittura, in c++ avrei dovuto scrivere const per indicare questo fatto
    long nelems;
    long *tsum; // puntatore alla variabile total_sum condivisa
    double *ttime; // puntatore alla variabile sum_time condivisa
} args_t;

int main() {
  pthread_t *thread_id;
  long n=1000000;
  long grain=300000; // grain operation per thread
  long nthreads;
  long *a;
  long total_sum = 0;
  ...
  for(long i=0; i < nthreads; i++) {
    ...
    arg->a = &(a[i*grain]); // passo il puntatore all'inizio del subarray
    arg->nelems = MIN(grain,(n-(i*grain))); // min tra grain e quanti elementi mancano
    arg->tsum = &total_sum; // passo il puntatore alla variabile total_sum condivisa
    arg->ttime = &sum_time;
