...
int main( int argc, char** argv ) {
    ...
    int rankheight = height/numranks; // height of image for each rank
    int rankele = rankheight*width; // number of elements for each rank
    ...
    // img segmentend in horizontal strips
    for(int i=0;i<numranks;i++){
        mystart[i] = i*rankheight; // 0 * 25, 1 * 25, 2 * 25, 3 * 25
        myend[i] = mystart[i]+rankheight; // 0+25, 25+25, 50+25, 75+25
    }
