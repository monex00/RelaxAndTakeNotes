...
int main( int argc, char **argv )
{
  ...
  if (myid == 0) {
	...
	while (true) {
	  ...
	  if (status.MPI_TAG==eos_tag) {
		...
		if (++n_eos>=(numprocs-1)) {
			std::cout << "Server going to Finalize\n";
	...
	while (rep<noreq) {
	  ...
	  MPI_Send(&request,1, MPI_INT,0 /* server */,msg_tag,MPI_COMM_WORLD);
	  MPI_Recv(&request,1, MPI_INT,0 /* server */,MPI_ANY_TAG,MPI_COMM_WORLD, &status);
  ...
  //MPI_Barrier(MPI_COMM_WORLD); // useless, just to show the barrier primitive
  t1 = MPI_Wtime();
